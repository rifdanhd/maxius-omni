...
import { Analytics202406GetShopSKUPerformanceDetailResponse } from '../model/analytics/V202406/GetShopSKUPerformanceDetailResponse';
...
export class AnalyticsV202406Api {
    ...
    /**
     * Returns performance metrics for the specified shop SKU over the requested date range.
     * @summary GetShopSKUPerformanceDetail
     * @param skuId The unique identifier of the SKU whose performance metrics are being queried.
     * @param startDateGe Start time (ISO 8601 YYYY-MM-DD format) in shop registered timezone, inclusive.
     * @param endDateLt End time (ISO 8601 YYYY-MM-DD format) in shop registered timezone, exclusive.
     * @param xTtsAccessToken 
     * @param contentType Allowed type: application/json
    ...
    public async ShopSkusSkuIdPerformanceGet (skuId: string, startDateGe: string, endDateLt: string, xTtsAccessToken: string, contentType: string, withComparison?: boolean, granularity?: string, currency?: string, shopCipher?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Analytics202406GetShopSKUPerformanceDetailResponse;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Analytics202406GetShopSKUPerformanceDetailResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Analytics202406GetShopSKUPerformanceDetailResponse");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
